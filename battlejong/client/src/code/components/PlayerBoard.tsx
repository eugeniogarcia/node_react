...
const PlayerBoard = ({ state }: any) => {
  ...
  //Cada ficha es un elemento react que se pintará como un elemento del virtual DOM de este componente React
  const tiles: ReactElement[] = [];

  let xOffset: number = 0;
  let yOffset: number = 0;

  //Recorremos cada una de las capas del tablero. El tablero tiene varios niveles con fichas. La primera dimensión hace referencia a cada nivel. La segunda sería cada fila de fichas en un determinado nivel, y la tercera una columna de fichas
    ...
    for (let r: number = 0; r < layer.length; r++) {
      ...
      for (let c: number = row.length; c >= 0; c--) {
        ...
        if (tileVal > 0) {
          ...
          //Cada ficha será un elemento react. Cada elemento tiene como propiedades la imagen - src -, el estilo - esto tiene que ser un valor de tipo CSSProperties -, la clase, y un event handler que asociamos al manejador incluido en el estado

          //El estilo lleva la posición de cada ficha
          ...
          //En función del valor de cada ficha, pintamos uno u otro elemento. Lo que cambia es la imagen que usamos. Cada elemento generado lo guardamos en el Array tiles
          switch (tileVal) {
