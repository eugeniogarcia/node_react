...
//Middleware para aceptar payloads json
app.use(express.json());


//Contenido estático. Aquí depositaremos nuestro cliente react
app.use("/", express.static(path.join(__dirname, "../../client/dist")));


//Middleware custom que implementa CORS
app.use(function(inRequest: Request, inResponse: Response, inNext: NextFunction) {
  //Origenes aceptados por CORS
  inResponse.header("Access-Control-Allow-Origin", "*");
  //Métodos aceptados por CORS
  inResponse.header("Access-Control-Allow-Methods", "GET,POST,DELETE,OPTIONS");
  //Cabeceras aceptadas por CORS
...
//El middleware no continua la cadena
app.get("/mailboxes",
...
app.post("/messages",
  async (inRequest: Request, inResponse: Response) => {
    ...
    try {
      ...
      let respuesta:string=await smtpWorker.sendMessage(inRequest.body);
      console.log("POST /messages: Ok");
      console.log(`POST /messages: ${respuesta}`);
      inResponse.send("ok");
    } catch (inError) {
